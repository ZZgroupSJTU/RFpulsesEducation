function showseqblockDynamic(block,showvalue,gifname)
...
axis([0,t(end),0,40]);
ax=gca;
set(ax,'XTick',linspace(0,t(end),5),'FontSize',12,'FontWeight','bold');
set(ax,'xticklabels',{num2str(0),[num2str(t(end)/4*1e3),' ms'],[num2str(t(end)/2*1e3),' ms'],...
                         [num2str(t(end)*3/4*1e3),' ms'], [num2str(t(end)*1e3),' ms']});
...
if nargin>2 && ~ischar('gifname')
    gifname='seqblockDynamic.gif';
end
for t_idx=1:N
    if t_idx>1
       delete(h);
    end
    h=line([t(t_idx),t(t_idx)],[0 40], 'LineWidth',2,  'Color','r','LineStyle','--' );  
    if (nargin>2)
        frame=getframe(gcf);
        im = frame2im(frame); 
